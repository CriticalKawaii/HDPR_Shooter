...
public class CasingSpawner : MonoBehaviour
{
    ...
    public void SpawnWithLocalDirection(Vector3 localDirection)
    {
        if (prefab == null)
        {
            Debug.LogError("Префаб не назначен!");
        ...
        // Рандомизируем переданное направление
        Vector3 randomizedDirection = RandomizeDirection(localDirection);
        Vector3 worldDirection = targetTransform.TransformDirection(randomizedDirection.normalized);
    ...
    // Метод для спавна с направлением относительно конкретного трансформа
    public void SpawnRelativeToTransform(Transform relativeTransform, Vector3 localDirection)
    {
        if (prefab == null)
        {
            Debug.LogError("Префаб не назначен!");
        ...
        {
            Debug.LogError("relativeTransform не назначен!");
            return;
        }

        Vector3 spawnPosition = relativeTransform.position;
        
        Quaternion baseRotation = matchSpawnPointRotation ? relativeTransform.rotation : Quaternion.identity;
        Quaternion finalRotation = baseRotation * Quaternion.Euler(rotationOffset) * Quaternion.Euler(-90f, 0f, 0f);
        ...
        // Рандомизируем переданное направление
        Vector3 randomizedDirection = RandomizeDirection(localDirection);
        Vector3 worldDirection = relativeTransform.TransformDirection(randomizedDirection.normalized);
        ...
        Debug.Log($"Префаб создан относительно {relativeTransform.name}. " +
                  $"Сила: {randomizedForce}, Направление: {worldDirection}");
    }

    // Метод для спавна в конкретной позиции (направление все равно относительно spawnPoint)
    public void SpawnAtPosition(Vector3 position)
    {
        ...
        GameObject spawnedObject = Instantiate(prefab, position, finalRotation);
        ...
        // Рандомизируем базовое направление
        Vector3 randomizedDirection = RandomizeDirection(impulseDirection);
        Vector3 worldDirection = targetTransform.TransformDirection(randomizedDirection.normalized);
    ...
    // Приватные методы для рандомизации
